...
Values controlled by the **Easy Mode** slider (return method, Goal Seek search settings, asset mix, market/balance triggers, glide spend timing, minimum withdrawal and consecutive-min recovery, gifting, spending timeline, and — when Goal Seek is off — the full spending plan) live in **`src/state/presets/balanced.json`** and the other preset files. See the next section.
