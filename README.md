...
1. **Picks the market's returns and inflation** using one of three methods you choose:
  ...
  - **Scaled historical:** uses the same real year-to-year sequences as resampling (same streaks, correlations, and crash patterns), but rescales each asset to the mean and volatility you set in the Return Assumptions panel instead of the raw historical average. Good when you want history's shape with different forward-looking assumptions.
