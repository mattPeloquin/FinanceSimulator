...
Values controlled by the **Easy Mode** slider (return method, Goal Seek search settings, asset mix, market triggers and the no-cut balance threshold, glide spend timing, minimum withdrawal and consecutive-min recovery, gifting, spending timeline, and — when Goal Seek is off — the full spending plan) live in **`src/state/presets/balanced.json`** and the other preset files. See the next section.
...
**Minimum withdrawals limit cuts, not the plan:** under the Base + Spending Over Time strategy, staged minimum-withdrawal tiers sit below your spending schedule and stop guardrails from cutting any single year too far — the chart shows plan and minimum separately. Goal Seek's risk tolerance governs how far below the *full planned schedule* lifetime spending is allowed to fall in bad markets. Optionally, **Max consecutive minimums** and **Then stay on plan for** return you to the full plan for a stretch after too many years at the minimum. With a **Specific List**, percentage minimum tiers work the same way — Goal Seek keeps your list fixed and only tunes the Market and Balance adjustment levers, but any percentage minimums you set still apply during the search.
